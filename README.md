...
### Towards a solution? ### 
The authors are working on open-sourcing the ASIO sdk much as VST2 sdk was. If you're interested in helping us, just drop an email or come and chat on Discord obs developper channel.      
...
(Be careful there are in the zip file x86 and x64 versions of the dll and lib ; pick the relevant versions according to your OBS-Studio binary.)    
...
* In cmake-gui, add the entry BASS\_ASIO\_INCLUDE\_DIR = path to the folder where bassasio.h is located.     
...
* In OBS-Studio folder, git checkout asiobass\_master.        
...
* Either drop bassasio.h in the folder $DepsPath/win64/include or $DepsPath/win32/include folder (given by DepsPath)    
    * OR indicate BASS\_ASIO\_INCLUDE\_DIR = path to the folder where bassasio.h is located.   
...
* If you followed the instructions carefully, you should end up with : 
    * rtaudio.dll and rtaudio\_static.lib in build/Release folder.    
...
* Git clone this repo : https://github.com/pkviet/obs-asio/
* git checkout asiort (to switch from master to RtAudio branch)
* The build instructions are similar to those given [here](https://github.com/Palakis/obs-websocket/blob/master/BUILDING.md).  
...
* Either drop rtaudio.h in the deps/win64/include or deps/win32/include folder (given by DepsPath)    
    * OR indicate RTAUDIO\_INCLUDE\_DIR = path to dir with rtaudio.h   
